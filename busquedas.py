...
def busqueda_A_estrella(problema, heuristica):
    ...
    contador = 0
    nodos_visitados = 0
    raiz = NodoBusqueda(s0)
    f0 = raiz.costo + heuristica(raiz)
    heapq.heappush(frontera, (f0, contador, raiz))
    ...
    while frontera:
        ...
        for hijo in nodo.expande(problema):
            g = hijo.costo
            if hijo.estado not in mejor_g or g < mejor_g[hijo.estado]:
                mejor_g[hijo.estado] = g
                contador += 1
                f = g + heuristica(hijo)
