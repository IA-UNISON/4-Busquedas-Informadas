...
def busqueda_primero_mejor(problema, f):
    ...
    """
    f = memoize(f) #optimizamos el calculo de la heuristica con memoize

    #pusheamos tublas en una lista mientras que heapq la mantiene como lista ordenada
    frontera = []
    heapq.heappush(frontera, (0, Nodo(problema.x0))) # inicamos caminos
    # Refenrencia - https://docs.python.org/3/library/heapq.html

    visitados = {problema.x0 : 0} # diccionario de nodos ya visitados

    while frontera:
        # retiramos un nodo, el costo esta dentro del nodo
        (_, nodo) = heapq.heappop(frontera)
